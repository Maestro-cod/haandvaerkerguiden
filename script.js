// ============================================================
// LISTINGS DATA — rigtige firmaer i Danmark
// ============================================================
const DEFAULT_LISTINGS = [

  // ── DIN FØRSTE RIGTIGE LISTING ──────────────────────────────
  { id:1, city:'nyborg', featured:true, name:"Maestro Tømrer & Snedker", trade:"tømrer", tradeLabel:"Tømrer", desc:"Erfaren tømrer med over 20 års erfaring. Gipsarbejde, facadebeklædning, vinduer og døre, gulve og tagarbejde. Arbejder i hele Danmark.", phone:"50 35 83 67", website:"https://hyrhaandvaerker.dk", area:"Nyborg, Fyn og hele Danmark", rating:5.0, reviewCount:1, lat:55.312, lng:10.790 },

  // Tilføj flere RIGTIGE firmaer herefter — eksempel:
  // { id:2, city:'odense', featured:false, name:"Firmanavn", trade:"elektriker", tradeLabel:"Elektriker", desc:"Beskrivelse", phone:"12 34 56 78", website:"", area:"Odense", rating:0, reviewCount:0, lat:55.396, lng:10.388 },
];
...
function getListings() {
  // v2: clear old fake data from previous version
  const ver = localStorage.getItem('hg_version');
  if (ver !== 'v2') {
    localStorage.removeItem('hg_listings');
    localStorage.setItem('hg_version', 'v2');
  }
...
const chatRules = [
  { keys:['hej','hallo','hey','goddag','hejsa'],
    ans:'Hej! 👋 Jeg er HyrHåndværkers assistent. Spørg mig om at finde håndværkere, priser, tilmelding og mere. Hvad kan jeg hjælpe med?' },
  { keys:['hvilke byer','hvilke by','dækker','område','dækning','byer'],
    ans:'Vi dækker <strong>hele Danmark</strong> 🇩🇰<br><br>Vi er et nyt site og udvider løbende. Er du håndværker? <a href="#bliv-listesat">Bliv listesat</a> og vær blandt de første!' },
  ...
  { keys:['tømrer','snedker','carport','terrasse'],
    ans:'Vi har en erfaren tømrer med 20+ års erfaring baseret i Nyborg, Fyn 🪵<br>📞 50 35 83 67<br><br>Brug filteret øverst for at finde flere.' },
  { keys:['elektriker','el-installatør','strøm','sikring','el '],
    ans:'Søger du elektriker? Vi udvider løbende med nye fagfolk. Brug filteret øverst til at søge.<br><br>Er du selv elektriker? <a href="#bliv-listesat">Bliv listesat</a> 🔌' },
  { keys:['vvs','badeværelse','toilet','rør','varme','varmepumpe'],
    ans:'Søger du VVS-installatør? Vi udvider løbende med nye fagfolk. Brug filteret øverst til at søge.<br><br>Er du selv VVS-mand? <a href="#bliv-listesat">Bliv listesat</a> 🔩' },
  { keys:['maler','maling','tapets'],
    ans:'Søger du maler? Vi udvider løbende. <a href="#bliv-listesat">Bliv listesat</a> som maler 🖌️' },
  { keys:['akut','haste','nødsituation','weekend'],
    ans:'Ring direkte til de listede firmaer og præcisér at det er akut 🚨' },
...
function toggleChat() {
  ...
  if (chatOpen && document.getElementById('chat-messages').children.length === 0) {
    addBubble('Hej! 👋 Jeg er HyrHåndværkers assistent. Spørg mig om håndværkere, priser eller tilmelding.', 'bot');
  }
...
function goToPayStep() {
  ...
  const comment = `${cPlan} HyrHaandvaerker`;
...
function submitJobRequest() {
  ...
  const body = encodeURIComponent(`Hej HyrHåndværker,\n\nJeg søger en ${trade} til følgende opgave:\n\n${desc}\n\nMine oplysninger:\n- Navn: ${name}${phone ? '\n- Telefon: '+phone : ''}\n\nVenlig hilsen,\n${name}`);
